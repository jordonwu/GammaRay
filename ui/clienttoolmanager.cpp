...
void GammaRay::ClientToolManager::setToolParentWidget(QWidget *parent)
{
    m_parentWidget = parent;
}

QWidget *GammaRay::ClientToolManager::widgetForId(const QString &toolId) const
{
    return widgetForIndex(toolIndexForToolId(toolId));
}

QWidget *GammaRay::ClientToolManager::widgetForIndex(int index) const
{
...
void ClientToolManager::gotTools(const GammaRay::ToolInfos &tools)
{
...
bool GammaRay::ClientToolManager::isToolListLoaded() const
{
    return m_tools.count();
}

QAbstractItemModel *GammaRay::ClientToolManager::model()
{
    if (!m_model)
...
QItemSelectionModel *GammaRay::ClientToolManager::selectionModel()
{
    if (!m_selectionModel)
...
void GammaRay::ClientToolManager::toolGotEnabled(const QString &toolId)
{
    int i = 0;
...
void GammaRay::ClientToolManager::toolGotSelected(const QString &toolId)
{
    emit toolSelected(toolId);
    emit toolSelectedByIndex(toolIndexForToolId(toolId));
}

int GammaRay::ClientToolManager::toolIndexForToolId(const QString &toolId) const
{
    int i = 0;
    auto it = m_tools.begin();
    for (; it != m_tools.end(); i++, it++) {
...
GammaRay::ClientToolManager::Model::Model(GammaRay::ClientToolManager *manager)
...
GammaRay::ClientToolManager::Model::~Model()
{
}
...
QVariant ClientToolManager::Model::data(const QModelIndex &index, int role) const
{
    ...
    const ToolInfo &tool = m_toolManager->toolAt(index.row());
...
void GammaRay::ClientToolManager::Model::toolEnabled(int toolIndex)
{
...
void GammaRay::ClientToolManager::Model::startReset()
{
    beginResetModel();
}

void GammaRay::ClientToolManager::Model::finishReset()
{
    endResetModel();
}
...
Qt::ItemFlags ClientToolManager::Model::flags(const QModelIndex &index) const
{
    ...
    ToolInfo tool = m_toolManager->toolAt(index.row());
    ToolUiFactory *factory = s_pluginRepository()->factories.value(tool.id);
...
int ClientToolManager::Model::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return m_toolManager->toolCount();
}

GammaRay::ClientToolManager::SelectionModel::SelectionModel(GammaRay::ClientToolManager *manager)
...
GammaRay::ClientToolManager::SelectionModel::~SelectionModel()
{
}

void GammaRay::ClientToolManager::SelectionModel::selectTool(int index)
{
...
void GammaRay::ClientToolManager::SelectionModel::selectDefaultTool()
{
